...
# Optional OpenAI API key with DRY_RUN fallback
...
def generate_qas_for_topic(topic: str, count: int) -> list[dict]:
	if DRY_RUN:
		# Deterministic local generation without OpenAI
		return [
			{
				"question": f"{topic}: sample question {i+1}?",
				"answer": f"This is a sample answer {i+1} for {topic}.",
				"topic": topic
			}
